...
import com.halfeaten.vaycray.databinding.FragmentStripePaymentBinding
...
class StripePaymentFragment : BaseFragment<FragmentStripePaymentBinding, PaymentViewModel>() {
            ...
            it?.let { outCome ->
                if (outCome == "1") {
                    viewModel.stripe.handleNextActionForPayment(
                        requireActivity(), viewModel.paymentIntentSecret.value!!
                    )
                }
            }
        })
    ...
    private fun creatingToken() {
        ...
        if (card == null) {
            ...
            viewModel.stripe.createPaymentMethod(card,
    ...
    private fun creatingToken(paymentMethod: String?, paymentIntent: String?) {
        ...
        if (paymentMethod != null) {
            //viewModel.createReservation(this)
        } else {
