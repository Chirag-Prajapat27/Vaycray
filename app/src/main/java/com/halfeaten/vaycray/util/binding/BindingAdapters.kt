package com.halfeaten.vaycray.util.binding
...
import com.halfeaten.vaycray.databinding.*
...
import devit951.github.magictip.activity
...
//        .build()!!
    ...
    @JvmStatic fun enableOrDisable(view: View, first: String?, last: String?, email: String?, password: String?, dob: String?, screen: AuthViewModel.Screen) {
        when(screen) {
    ...
    @JvmStatic fun rating(view: RatingBar, rating :Float,viewmodel:ReviewViewModel) {
    ...
    @JvmStatic fun nameSpan(view: TextView, name: String?, title: String?, type: String?, listId: Int?, viewModel: ReviewViewModel?, isAdmin: Boolean?, isListing: Boolean?) {
        try{
...
//                         }
    ...
    @JvmStatic fun textSpan(view: TextView, name: String?,start: Int,end: Int) {
    ...
    @JvmStatic fun loadInboxProfileImage(view: ImageView, url: String?, isAdmin: Boolean = false) {
        if (isAdmin) {
    ...
    @JvmStatic fun loadInboxProfileSenderImage(view: ImageView, url: String?, isAdmin: Boolean = false) {
        if (isAdmin) {
            GlideApp.with(view.context).load(R.drawable.ic_account_verified).transform(CenterCrop(), GranularRoundedCorners(25f, 25f, 25f, 0f)).into(view)
        } else {
            Timber.d("urlImageNotUpdated--->>  $url")
    ...
    @JvmStatic fun loadInboxProfileReceiverImage(view: ImageView, url: String?, isAdmin: Boolean = false) {
        if (isAdmin) {
            GlideApp.with(view.context).load(R.drawable.ic_account_verified).transform(CenterCrop(), GranularRoundedCorners(25f, 25f, 25f, 0f)).into(view)
        } else {
            Timber.d("urlImageNotUpdated--->>  $url")
    ...
    @BindingAdapter("imgActualUrl")
    @JvmStatic fun loadImgActualUrl(view: ImageView, url: String?) {
        url?.let {
            if (url.isNotEmpty()) {
                Timber.d("Normal image-->>" + Constants.imgListingMedium + url)
                Timber.d("Small image---->>" + Constants.imgListingSmall + url)
                GlideApp.with(view.context)
                    .load(Constants.imgListing + url)
                    .transform(CenterInside(), GranularRoundedCorners(0f, 0f, 0f, 0f))
                    .thumbnail(
                        GlideApp.with(view.context)
                        .load(Constants.imgListingSmall + url)
                        .thumbnail(
                        ...
                        .transform(BlurTransformation(30, 3), CenterInside(), GranularRoundedCorners(0f, 0f, 0f, 0f))
                        .diskCacheStrategy(DiskCacheStrategy.ALL)
                    )
                    .transition(DrawableTransitionOptions.withCrossFade(DrawableCrossFadeFactory.Builder().setCrossFadeEnabled(true).build()))
                    .diskCacheStrategy(DiskCacheStrategy.ALL)
    ...
    @JvmStatic fun loadListingImageZoom(view: ImageView, url: String?) {
        url?.let {
            if (url.isNotEmpty()) {
                GlideApp.with(view.context)
                    . load(Constants.imgListing + url)
                    ...
                    .thumbnail(
                        GlideApp.with(view.context)
                            ...
                            .thumbnail(
                                GlideApp.with(view.context)
                                    .load(R.drawable.bg_loading_image_placeholder)
    ...
    @JvmStatic fun loadBecomeHostBaner(view: ImageView, url: String?) {
        url?.let {
            if (url.isNotEmpty()) {
                GlideApp.with(view.context)
                    .load(url)
    ...
    @JvmStatic fun loadBecomeHostBanerGradient(view: ImageView, url: String?) {
        url?.let {
            if (url.isNotEmpty()) {
                GlideApp.with(view.context)
                        .load(url)
    ...
    @JvmStatic fun loadImgSimpleUrl(view: ImageView, url: String?) {
        url?.let {
            if (url.isNotEmpty()) {
                Timber.d("Normal image-->>" + Constants.imgListingMedium + url)
                Timber.d("Small image---->>" + Constants.imgListingSmall + url)
    ...
    @JvmStatic fun instantBook1(view: TextView, bookingType: String?,start: Int,end: Int,pricee: String?) {
    ...
    @JvmStatic fun textColor(view: TextView, color: Int?) {
        color?.let {
    ...
    @JvmStatic fun statusInbox(view: ImageView, status: String?) {
        when(status){
    ...
    @JvmStatic fun fontFamily(view: TextView, text: String?) {
        text?.let {
    ...
    @JvmStatic fun enableSendImageBtn(view: ImageView, text: String?) {
        text?.let {
            if (text.isNotEmpty() && text.trim().isNotEmpty()) {
                view.EnableAlpha(true)
            } else {
    ...
    @JvmStatic fun iconColor(view: ImageView, color: Boolean){
        color.let {
            if(color){
    ...
    @JvmStatic fun imageTint(view: ImageView, color: Boolean){
        Glide.with(view)
            .asBitmap()
            .load(view.drawable)
    ...
    @JvmStatic fun modeImage(view: ImageView,boolean: Boolean){
    ...
    @JvmStatic fun imgVisible(view: ImageView, status: String){
        status.let {
            if(status.equals("active")){
                view.visibility = View.VISIBLE
            }else if(status.equals("completed")){
    ...
    @JvmStatic fun imgVisible(view: View, status: String){
        status.let {
            if(status.equals("active")){
                view.visibility = View.VISIBLE
            }else if(status.equals("completed")){
    ...
    @JvmStatic fun imgVisible(view: RelativeLayout, status: String){
        status.let {
            if(status.equals("active")){
                view.visibility = View.VISIBLE
            }else if(status.equals("completed")){
    ...
    @JvmStatic fun arrow(view: ImageView, imgVisibility: Int){
      if (imgVisibility==1){
    ...
    @JvmStatic fun drawableAmenities(view: ImageView, image: String?){
    ...
    @JvmStatic fun drawableAmenitiesBlack(view: ImageView, image: String?,isChecked: Boolean){
        if (image==null||image.isEmpty()) {
            if (isChecked) {
    ...
    @JvmStatic fun chipStatus(view: Chip, status: Boolean?,chip:Int){
    ...
    @BindingAdapter(value = ["chip1","chip2","chip3","chip4","chip5","chip6"
        ,"chip7","chip8","chip9","chip10"], requireAll = false)
    @JvmStatic fun scrollPlaceType(view: HorizontalScrollView, chip1: Chip?, chip2: Chip?, chip3: Chip?
                                   , chip4: Chip?, chip5: Chip?, chip6: Chip?, chip7: Chip?, chip8: Chip?, chip9: Chip?, chip10: Chip?){
        if (chip1!=null) {
    ...
    @JvmStatic fun bedsPlural(view: TextView, count: Int?){
